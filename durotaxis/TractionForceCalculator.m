...
x_cell.location_x = mod(x_cell.location_x - p.bottom_scale - 30,p.domain_scale_x - 60) + p.bottom_scale + 30;
x_cell.location_y = mod(x_cell.location_y - p.bottom_scale - 30,p.domain_scale_y - 60) + p.bottom_scale + 30;
x_cell.location_z = mod(x_cell.location_z - p.bottom_scale - 30,p.domain_scale_z - 60) + p.bottom_scale + 30;
...
%     end
    ...
    F.frictionECM(i,1) = p.cresistance*realsqrt(sum(x_cell.v(i,:).^2,2))*sum(p.kbend(all_contact_index).*theta_bend.*sin_theta.*dir_perpen_f_norm(:,1)./p.fiber_length(all_contact_index));
    F.frictionECM(i,2) = p.cresistance*realsqrt(sum(x_cell.v(i,:).^2,2))*sum(p.kbend(all_contact_index).*theta_bend.*sin_theta.*dir_perpen_f_norm(:,2)./p.fiber_length(all_contact_index));
    F.frictionECM(i,3) = p.cresistance*realsqrt(sum(x_cell.v(i,:).^2,2))*sum(p.kbend(all_contact_index).*theta_bend.*sin_theta.*dir_perpen_f_norm(:,3)./p.fiber_length(all_contact_index));
end