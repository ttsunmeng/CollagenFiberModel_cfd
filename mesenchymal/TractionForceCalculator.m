...
%     end
    ...
    F.frictionECM(i,1) = p.cresistance*p.kbend*sum(theta_bend.*sin_theta.*dir_perpen_f_norm(:,1)./p.fiber_length(all_contact_index));
    F.frictionECM(i,2) = p.cresistance*p.kbend*sum(theta_bend.*sin_theta.*dir_perpen_f_norm(:,2)./p.fiber_length(all_contact_index));
    F.frictionECM(i,3) = p.cresistance*p.kbend*sum(theta_bend.*sin_theta.*dir_perpen_f_norm(:,3)./p.fiber_length(all_contact_index));
