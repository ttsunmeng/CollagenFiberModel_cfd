function p = cfd_initiation_flow_May17th_02mg(filename,k,kk)
    ...
    if k ~= 0
        ...
        chemotaxis_table = [0.434,0.436,0.439,0.441,0.444,0.447,0.45;0.403,0.407,0.411,0.415,0.419,0.423,0.427;0.309,0.308,0.308,0.308,0.308,0.307,0.307;0.297,0.296,0.293,0.29,0.288,0.285,0.282;0.28,0.279,0.275,0.271,0.267,0.263,0.259];
        ...
        p.flow_v = [0 0 flow_list(k)]; % um/s from Polacheck et al 2011 PNAS.
        p.flow_v_z = flow_list(k);
        chemotaxis_table = [0.432,0.433,0.435,0.438,0.44,0.443,0.449;0.403,0.405,0.409,0.413,0.417,0.421,0.424;0.309,0.308,0.308,0.308,0.308,0.307,0.307;0.297,0.296,0.293,0.29,0.288,0.285,0.282;0.28,0.279,0.275,0.271,0.267,0.263,0.259];
        flow_v_table = [0.3;0.5;1;1.5;2;2.5;3];
        theta_table = [0;45;90;135;180];
        [X,Y] = meshgrid(flow_v_table,theta_table);
        [Xq,Yq] = meshgrid(norm(p.flow_v),(0:1:180)');
        ...
        p.flow_v = [0 0 flow_list(k)]; % um/s from Polacheck et al 2011 PNAS.
        p.flow_v_z = flow_list(k);
        chemotaxis_table = [0.43,0.431,0.433,0.435,0.438,0.441,0.446;0.403,0.405,0.409,0.413,0.417,0.421,0.424;0.309,0.308,0.308,0.308,0.308,0.307,0.307;0.297,0.296,0.293,0.29,0.288,0.285,0.282;0.28,0.279,0.275,0.271,0.267,0.263,0.259];
        flow_v_table = [0.3;0.5;1;1.5;2;2.5;3];
        theta_table = [0;45;90;135;180];
        [X,Y] = meshgrid(flow_v_table,theta_table);
        [Xq,Yq] = meshgrid(norm(p.flow_v),(0:1:180)');
    ...
    p.mag_protursion = 100e-12; % 50-500pN: half goes to protursion (50pN) and half to traction?
...
%     p.ctraction_add = 0.579*p.flow_v_z/(0.857 + p.flow_v_z); % the percentage of traction upstream increase at 0.3um/s
